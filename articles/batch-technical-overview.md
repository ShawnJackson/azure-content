...
Azure Batch helps you run large-scale parallel and high-performance computing (HPC) applications in the cloud by providing job scheduling and autoscaling of compute resources as a platform service. By using the Batch software development kit (SDK) and Batch service, you can configure batch workloads to run on demand or on a schedule on a managed collection of virtual machines. Then, you won't have to worry about the complexity of scheduling jobs and managing resources in the underlying platform.
...
Batch works well with intrinsically parallel (sometimes called "embarrassingly parallel") applications or workloads, which lend themselves to running as parallel tasks on multiple computers, such as the compute virtual machines that the Batch service manages. Figure 1 illustrates parallel tasks.  
...
The REST-based Batch APIs support two developer scenarios to help you configure and run your batch workloads by using the Batch service:
 
1. **Distribute computations as work items** – Use the *Batch* APIs to create and manage a flexible pool of compute virtual machines and specify work items that run on them. This gives you full control over resources and requires the client to manage the task execution pipeline--for example, with a workflow manager or meta-scheduler, which can be implemented through the Batch REST APIs or optionally a job manager feature of the work item. Instead of having to set up a compute cluster or write code to queue and schedule your jobs, you automate the scheduling of compute jobs and scale a pool of compute virtual machines up and down to run them. As part of specifying work items, you define all dependencies and define the movement of input and output files. 

2. **Publish and run applications by using the Batch service** – The *Batch Apps* APIs provide a higher level of abstraction and a job execution pipeline that the Batch service hosts. By using Batch Apps, you can create a batch workload as a service in the cloud from an application that runs today on client workstations or a compute cluster. Batch Apps helps you wrap existing binaries and executable (.exe) files and publish them to run on pooled virtual machines that the Batch service creates and manages in the background. The Batch Apps framework handles the movement of input and output files, job execution, job management, and data persistence. You can also use Batch Apps to model tasks for how data is partitioned and for multiple steps in a job. Included is a REST-based API and the Batch Apps portal, which can be accessed from the Azure management portal and helps you monitor the jobs you submitted.
...
You need a unique **Batch account** to work with the Batch service. When you perform API operations with the Batch service, you authenticate your requests with the URL of the account and an associated access key. You also need a Batch account to access the Batch Apps portal.
...
A **task virtual machine** (TVM) is an Azure virtual machine that the Batch service dedicates to running a specific workload (task) for your application--such as an executable file, or in the case of Batch Apps, one or more programs from an application image. Unlike a typical Azure virtual machine, you don't provision or manage a TVM directly. Instead, the Batch service creates and manages TVMs as a **pool** of similarly configured compute resources, as shown in Figure 2. 

![Pool and TVMs][TVM_pool]

**Figure 2. A pool of TVMs**

If you use the Batch APIs, you can create a pool directly, or you can configure the Batch service to create one automatically when you specify the work to be done. If you use the Batch Apps APIs, a pool is created automatically when you run your cloud-enabled Batch application.
...
* A start task for TVMs, for one-time operations like installing applications or downloading data that tasks use

>[WACOM.NOTE]Currently, a TVM can run only the Windows Server 2012 R2, Windows Server 2012, or Windows Server 2008 R2 SP1 operating system.

A pool can be used only by the Batch account in which it was created. A Batch account can have more than one pool.
...
* If you develop an application by using the lower-level Batch APIs, you need to programmatically define all the work items, jobs, and tasks that the Batch service runs and configure the TVM pools that run the tasks.
...
A file is a piece of data used as an input to a job task. A task can have zero, one, or many input files associated with it. The same file can be used in multiple tasks. For example, the tasks in a movie rendering job might include commonly used textures and models. For tasks in a data analysis job, the files might be a set of observations or measurements.

Each task has a working directory under which it creates directories and files needed to store the program that a task runs, the input data that a task processes, and the output of the processing that a task performs. These directories and files are available for other tasks to use while a job runs. A single user account owns all tasks, files, and directories on a TVM.
...
3.	Create a pool of TVMs, specifying the size of the TVMs in the pool, the operating system they run, and other properties. When a task runs, it is assigned a TVM from this pool.
4.	Create a work item. A job will be automatically created when you create a work item. You can use the work item to manage a job of tasks.
...
**Figure 5. Workflow to publish and run an application by using Batch Apps**

1.	Prepare an **application image**--a compressed (.zip) file of your existing application executable files and any support files they need. These might be the same executable files you run in a traditional server farm or cluster.
